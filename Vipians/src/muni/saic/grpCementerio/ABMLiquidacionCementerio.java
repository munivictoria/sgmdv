 ...
 * 
 */
public class ABMLiquidacionCementerio extends ABMPageBean {
	 ...
	 * </p>
	 */
	public ABMLiquidacionCementerio() {
	}

	protected ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		int ind = 0;
		ep.getObjetos().add(ind++, null); // Liquidacion Cementerio
	...
	protected void mostrarEstadoObjetosUsados() {
		LiquidacionTasa liquidacionCementerio = (LiquidacionTasa) this.obtenerObjetoDelElementoPila(0);

		if(liquidacionCementerio != null) {
			this.getTfPeriodo().setText(liquidacionCementerio.getPeriodoAnio().toString());
			this.getTaFormula().setText(liquidacionCementerio.getStringFormula());
			this.getTaParametros().setText(liquidacionCementerio.getStringParametrosValuados());
			this.getTaParametrosValuadosAlicuota().setText(liquidacionCementerio.getStringParametrosValuadosAlicuota());
			this.getTaModificadores().setText(liquidacionCementerio.getStringModificadoresLiquidacion());
			this.getTaVencimientos().setText(liquidacionCementerio.getStringVencimientos());
			if(liquidacionCementerio.getRegistroExencionRegistroDeuda() != null) {
				this.getTaExencion().setText(liquidacionCementerio.getRegistroExencionRegistroDeuda().getExencionRegistroDeuda().toString());
			}

			this.getTfFechaObligacion().setText(liquidacionCementerio.getFechaEmision());
			/* this.getTfPersona().setText(liquidacionTGI.getPersona()); */
			this.getTaObligacion().setText(liquidacionCementerio.getStringObligacion());
			this.getTfCuotaLiquidada().setText(liquidacionCementerio.getCuotaLiquidada());
			this.getTfEstado().setText(liquidacionCementerio.getEstado().toString());
			this.getTfMonto().setText(Conversor.getStringDeDouble(liquidacionCementerio.getMonto()));
			if(liquidacionCementerio.getFechaCancelacion() != null) {
				this.getTfFechaCancelacion().setText(liquidacionCementerio.getFechaCancelacion());
			}
		}
	}
	...
	protected void procesarObjetoABM(Object pObject) {
		LiquidacionTasa liquidacionCementerio = (LiquidacionTasa) pObject;
		try {
			this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().setLlave(this.getSessionBean1().getLlave());
			liquidacionCementerio = (LiquidacionTasa) this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa()
					.getRegistroDeudaPorId(new Long(liquidacionCementerio.getIdRegistroDeuda()));
		} catch(Exception e) {
			e.printStackTrace();
		}

		this.getElementoPila().getObjetos().set(0, liquidacionCementerio);
	}
	...
	public String getNombreBean() {
		return "#{saic$grpCementerio$ABMLiquidacionCementerio$ABMLiquidacionCementerio}";
	}
