...
public class GenerarLiquidacionCementerio extends AbstractPageBean {
	...
	private final String NOMBRE_PAGINA = "Generar Liquidaciones de Cementerio";
	// nombre del caso de navegacion para llegar a esta pagina.
	private final String CASO_NAVEGACION = "GenerarLiquidacionCementerio";
	 ...
	 */
	public GenerarLiquidacionCementerio() {
	}
	...
	private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		...
		ep.getObjetos().add(4, new ParcelaCementerio());
		ep.getObjetos().add(5, getAnioCorriente().toString()); // 5 anios
		ep.getObjetos().add(6, null); // 6 tipo sepultura
		ep.getObjetos().add(7, new ArrayList()); // 7 lista atributos dinamicos
	...
	private void guardarEstadoObjetosUsados() {
		...
		ParcelaCementerio parcela = (ParcelaCementerio) this.obtenerObjetoDelElementoPila(4, ParcelaCementerio.class);
		List listaAtributosDinamicos = (List) this.obtenerObjetoDelElementoPila(7, ArrayList.class);

		if (persona != null && persona.getIdPersona() == -1) {
			persona = null;
		}
		if (parcela != null && parcela.getIdAsocRegAlicuota() == -1) {
			parcela = null;
		}
		...
		this.getElementoPila().getObjetos().set(6, tipoSepulturaSeleccionada);
		this.getElementoPila().getObjetos().set(7, listaAtributosDinamicos);
	...
	private void mostrarEstadoObjetosUsados() {
		...
		ParcelaCementerio parcela = null;

		this.acomodarSeleccionado();

		persona = (Persona) this.obtenerObjetoDelElementoPila(3, Persona.class);
		parcela = (ParcelaCementerio) this.obtenerObjetoDelElementoPila(4, ParcelaCementerio.class);
		String anio = (String) this.obtenerObjetoDelElementoPila(5, String.class); 
		String tipoSepultura = (String) this.obtenerObjetoDelElementoPila(6, String.class);
		List listaAtributosDinamicos = (List) this.obtenerObjetoDelElementoPila(7, ArrayList.class);
		...
		if (tipoSepultura != null){
			this.getDdTipoSepultura().setSelected(tipoSepultura);
		}
		...
		if (listaAtributosDinamicos.isEmpty()) {
			try {
				listaAtributosDinamicos =  this.getComunicationBean()
						.getRemoteSystemParametro().getAtributosPorRecurso(DocumentoCementerio.serialVersionUID, null, true);
		...
		panelAtributoDinamico = new PanelAtributoDinamico(listaAtributosDinamicos,"#{saic$grpCementerio$ABMLiquidacionCementerio$GenerarLiquidacionCementerio}");
	...
	public String btnGenerarLiquidaciones_action() {
		...
		if (ultimo) {
			...
			try {
				...
				TipoSepultura locTipoSepultura = getApplicationBean1().getMapaTipoSepultura().get(tipoSepultura);
				...
				if (parcela.getIdAsocRegAlicuota() == -1) {
					parcela = null;
				}
               ...
               ResultadoLiquidacion locResultadoLiquidacion = this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().liquidarCementerio(parcela, locCuota, persona, locTipoSepultura);
               ...
               totalGenerados = locResultadoLiquidacion.getCantidadLiquidadas();
				...
				case 0:
					mensaje = "No se generaron Liquidaciones de Cementerio.";
					break;
				case 1:
					mensaje = "Se gener\363 exitosamente 1 Liquidaci\363n de Cementerio.";
					break;
				default:
					mensaje = "Se generaron exitosamente " + totalGenerados + " Liquidaciones de Cementerio.";
				}
	...
	public String btnSeleccionarParcela_action() {
		...
		if (ultimo) {
			...
			// CAMBIAR: Caso de navegacion de la pagina de administracion correspondiente.
			retorno = "AdminParcelaCementerio";
		} else {
			retorno = this.prepararCaducidad();
		}
	...
	private CuotaLiquidacion getRecuperarCuotaPorPeriodo( String pCuota ){
		CuotaLiquidacion locCuota = this.getCommunicationSAICBean().getMapaCuotasCalendarioMunicipalCementerio(null).get(pCuota);
		return locCuota;
	}
