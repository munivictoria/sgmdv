 ...
 */
public class AutorizarExencionObligacion extends AbstractPageBean {
    ...
    private void _init() throws Exception {
        ...
        Option[] op2 = null;
        op2 = this.getApplicationBean1().getMgrDropDown().armarArrayOptions(ExencionObligacion.Tipo.values(),"cap");

        ddTipoDefaultOptions.setOptions(op2);
    ...
    private ObjectListDataProvider ldpExencionFirma = new ObjectListDataProvider();

    public ObjectListDataProvider getLdpExencionFirma() {
        return ldpExencionFirma;
    }

    public void setLdpExencionFirma(ObjectListDataProvider oldp) {
        this.ldpExencionFirma = oldp;
    }
    ...
    private StaticText stComentario = new StaticText();

    public void setStComentario(StaticText stComentario) {
        this.stComentario = stComentario;
    }

    public StaticText getStComentario() {
        return stComentario;
    }
    private TableColumn tcFechaHora = new TableColumn();

    public TableColumn getTcFechaHora() {
        return tcFechaHora;
    }

    public void setTcFechaHora(TableColumn tcFechaHora) {
        this.tcFechaHora = tcFechaHora;
    }

    private StaticText stFechaHora = new StaticText();

    public void setStFechaHora(StaticText stFechaHora) {
        this.stFechaHora = stFechaHora;
    }

    public StaticText getStFechaHora() {
        return stFechaHora;
    }
    ...
    private final String NOMBRE_PAGINA = "Consultar Exencion";
    // nombre del caso de navegacion para llegar a esta pagina.
    private final String CASO_NAVEGACION = "ConsultarExencionObligacion";
     ...
     */
    public AutorizarExencionObligacion() {
    }
    ...
    public void init() {
        ...
        // TODO - Consultar código de inicio propio aquí

        // <editor-fold defaultstate="collapsed" desc="Inicio de componente administrado por el programa">
        // Iniciar componentes administrados automáticamente
        // *Nota* - esta lógica NO debe Consultarse
        try {
            _init();
        } catch (Exception e) {
            log("ConsultarExencion Initialization Failure", e);
    ...
    private ElementoPila ConsultarObjetosAElementoPila(ElementoPila ep) {
        int ind = 0;
    ...
    private void guardarEstadoObjetosUsados() {
        ...
        int ind=0;
        ExencionObligacion ExencionObligacion = (ExencionObligacion) this.obtenerObjetoDelElementoPila(ind++, ExencionObligacion.class);
        ...
        if (nombre != null && nombre != "") ExencionObligacion.setNombre(nombre.toString());
        else ExencionObligacion.setNombre(null);
        if (porcentaje != null && porcentaje != "") ExencionObligacion.setPorcentaje(new Double(porcentaje.toString()));
        else ExencionObligacion.setPorcentaje(null);
        if (motivo != null && motivo != "") ExencionObligacion.setMotivo(motivo.toString());
        else ExencionObligacion.setMotivo(null);
...
//       Comentado
//       ExencionObligacion.setPeriodo(periodo);


       if (digestoMunicipal.getIdDigestoMunicipal() == -1) digestoMunicipal = null;
       if (digestoMunicipal != null)ExencionObligacion.setDigestoMunicipal(digestoMunicipal);

       this.getLdpExencion().commitChanges();

       ind = 0;
       this.getElementoPila().getObjetos().set(ind++, ExencionObligacion);
    ...
    private void mostrarEstadoObjetosUsados() throws Exception {
        ...
        if (this.getRequestBean1().getObjetoABM() != null) {
            try{
                locExencionObligacion  = (ExencionObligacion) this.getRequestBean1().getObjetoABM();
            }catch(Exception e){
                log(CASO_NAVEGACION + "_Guardar Error:", e);
                e.printStackTrace();
            }

            digesto = (DigestoMunicipal)locExencionObligacion.getDigestoMunicipal();
            
//            Comentado
//            anio = new Integer(locExencionObligacion.getPeriodo().getFechaInicio().get(Calendar.YEAR));
//            periodicidad = locExencionObligacion.getPeriodo().getPeriodicidad();
//            periodoNumero = locExencionObligacion.getPeriodo().getNumeroPeriodo();
     
            this.getElementoPila().getObjetos().set(0, locExencionObligacion);
       ...
       this.tfPorcentaje.setText(locExencionObligacion.getPorcentaje());
       this.taMotivo.setText(locExencionObligacion.getMotivo());

       if (locExencionObligacion.getTipoExencion() != null){
            this.getDdTipo().setSelected(Util.getEnumNameFromString((locExencionObligacion.getTipoExencion().toString())));
            this.getDdTipoDefaultOptions().setSelectedValue(Util.getEnumNameFromString((locExencionObligacion.getTipoExencion().toString())));
       }
     ...
     //periodo:
       ...
       if (locExencionObligacion.getTipoExencion().equals(ExencionObligacion.Tipo.PERIODO)){
       ...
       listaExenciones.addAll(locExencionObligacion.getListaRegistrosExencion());
       this.getLdpExencion().setList(listaExenciones);
       this.setListaDelCommunication(listaExenciones);

       ArrayList listaFirmas = new ArrayList();
       listaFirmas.addAll(locExencionObligacion.getListaFirmas());
       this.getLdpExencionFirma().setList(listaFirmas);
       this.setListaDelCommunication2(listaFirmas);
    ...
    private ObjectListDataProvider getObjectListDataProvider2() {
        // CAMBIAR: Utilizar el ObjectListDataProvider correspondiente
        return this.getLdpExencionFirma();
    }

    private ArrayList getListaDelCommunication2() {
        // CAMBIAR: Utilizar la Lista del Comunication que corresponda
        return this.getCommunicationSAICBean().getListaExencionFirmas();
    }

    private void setListaDelCommunication2(ArrayList lista) {
        // CAMBIAR: Utilizar la Lista del Comunication que corresponda
        this.getCommunicationSAICBean().setListaExencionFirmas(lista);
    }
    // </editor-fold>
    ...
    private void crearElementoPila() {
        ...
        ep = this.ConsultarObjetosAElementoPila(ep);
...
// </editor-fold>
    ...
    public String btnAutorizar_action() throws RemoteException {
        String retorno = null;
        ...
        if (ultimo) {
            try {
                ...
                this.getCommunicationHabilitacionesBean().getRemoteSystemExencion().autorizarExencionObligacion((ExencionObligacion) locExencionObligacion);
                this.getRequestBean1().setRespuestaABM(locExencionObligacion);
                info("La Exenci\363n se autoriz\363 exitosamente.");
    ...
    private void refrescarTabla() throws Exception{
        ...
        ExencionObligacion locExencionObligacion= (ExencionObligacion) this.obtenerObjetoDelElementoPila(0, ExencionObligacion.class);
        ArrayList listaObligacion= new ArrayList();
        listaObligacion.addAll(locExencionObligacion.getListaRegistrosExencion());
        ...
        try{
             ...
             this.setListaDelCommunication(listaObligacion);
             this.getLdpExencion().setList(this.getListaDelCommunication());

             this.setListaDelCommunication2(listaFirmas);
             this.getLdpExencionFirma().setList(this.getListaDelCommunication2());

        }catch(Exception ex){
            this.getLdpExencion().setList(null);
            this.getLdpExencionFirma().setList(null);
        }
    }
