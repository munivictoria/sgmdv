 ...
 * 
 */
public class ABMLiquidacionAutomotor extends ABMPageBean {
	 ...
	 * </p>
	 */
	public ABMLiquidacionAutomotor() {
	}

	protected ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		int ind = 0;
		ep.getObjetos().add(ind++, null); // Liquidacion Automotor
	...
	protected void mostrarEstadoObjetosUsados() {
		LiquidacionTasa liquidacionAutomotor = (LiquidacionTasa) this.obtenerObjetoDelElementoPila(0);

		if(liquidacionAutomotor != null) {
			this.getTfPeriodo().setText(liquidacionAutomotor.getPeriodoAnio().toString());
			this.getTaFormula().setText(liquidacionAutomotor.getStringFormula());
			this.getTaParametros().setText(liquidacionAutomotor.getStringParametrosValuados());
			this.getTaModificadores().setText(liquidacionAutomotor.getStringModificadoresLiquidacion());
			this.getTaVencimientos().setText(liquidacionAutomotor.getStringVencimientos());
			if(liquidacionAutomotor.getRegistroExencionRegistroDeuda() != null) {
				this.getTaExencion().setText(liquidacionAutomotor.getRegistroExencionRegistroDeuda().getExencionRegistroDeuda().toString());
			}

			this.getTfFechaObligacion().setText(liquidacionAutomotor.getFechaEmision());
			/* this.getTfPersona().setText(liquidacionTGI.getPersona()); */
			this.getTaObligacion().setText(liquidacionAutomotor.getStringObligacion());
			this.getTfCuotaLiquidada().setText(liquidacionAutomotor.getCuotaLiquidada());
			this.getTfEstado().setText(liquidacionAutomotor.getEstado().toString());
			this.getTfMonto().setText(Conversor.getStringDeDouble(liquidacionAutomotor.getMonto()));
			if(liquidacionAutomotor.getFechaCancelacion() != null) {
				this.getTfFechaCancelacion().setText(liquidacionAutomotor.getFechaCancelacion());
			}
		}
	}
	...
	protected void procesarObjetoABM(Object pObject) {
		LiquidacionTasa liquidacionAutomotor = (LiquidacionTasa) pObject;
		try {
			this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().setLlave(this.getSessionBean1().getLlave());
			liquidacionAutomotor = (LiquidacionTasa) this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa()
					.getRegistroDeudaPorId(new Long(liquidacionAutomotor.getIdRegistroDeuda()));
		} catch(Exception e) {
			e.printStackTrace();
		}

		this.getElementoPila().getObjetos().set(0, liquidacionAutomotor);
	}
	...
	public String getNombreBean() {
		return "#{saic$grpAutomotor$ABMLiquidacionAutomotor$ABMLiquidacionAutomotor}";
	}
