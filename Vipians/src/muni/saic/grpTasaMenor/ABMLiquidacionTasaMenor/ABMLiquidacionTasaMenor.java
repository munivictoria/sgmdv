 ...
 * 
 */
public class ABMLiquidacionTasaMenor extends ABMPageBean {
	 ...
	 * </p>
	 */
	public ABMLiquidacionTasaMenor() {
	}

	protected ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		int ind = 0;
		ep.getObjetos().add(ind++, null); // Liquidacion Tasa Menor
	...
	protected void mostrarEstadoObjetosUsados() {
		LiquidacionTasa liquidacionTasaMenor = (LiquidacionTasa) this.obtenerObjetoDelElementoPila(0);

		if(liquidacionTasaMenor != null) {
			this.getTaParametrosValuadosAlicuota().setText(liquidacionTasaMenor.getStringParametrosValuadosAlicuota());
			this.getTaFormula().setText(liquidacionTasaMenor.getStringFormula());
			this.getTaParametros().setText(liquidacionTasaMenor.getStringParametrosValuados());
			this.getTaModificadores().setText(liquidacionTasaMenor.getStringModificadoresLiquidacion());
			this.getTaVencimientos().setText(liquidacionTasaMenor.getStringVencimientos());
			if(liquidacionTasaMenor.getRegistroExencionRegistroDeuda() != null) {
				this.getTaExencion().setText(liquidacionTasaMenor.getRegistroExencionRegistroDeuda().getExencionRegistroDeuda().toString());
			}

			this.getTfFechaObligacion().setText(liquidacionTasaMenor.getFechaEmision());
			this.getTaObligacion().setText(liquidacionTasaMenor.getStringObligacion());
			this.getTfCuotaLiquidada().setText(liquidacionTasaMenor.getCuotaLiquidada());
			this.getTfEstado().setText(liquidacionTasaMenor.getEstado().toString());
			this.getTfMonto().setText(Conversor.getStringDeDouble(liquidacionTasaMenor.getMonto()));
			if(liquidacionTasaMenor.getFechaCancelacion() != null) {
				this.getTfFechaCancelacion().setText(liquidacionTasaMenor.getFechaCancelacion());
			}
		}
	}
	...
	protected void procesarObjetoABM(Object pObject) {
		LiquidacionTasa liquidacionTasaMenor = (LiquidacionTasa) pObject;
		try {
			this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().setLlave(this.getSessionBean1().getLlave());
			liquidacionTasaMenor = (LiquidacionTasa) this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa()
					.getRegistroDeudaPorId(new Long(liquidacionTasaMenor.getIdRegistroDeuda()));
		} catch(Exception e) {
			e.printStackTrace();
		}

		this.getElementoPila().getObjetos().set(0, liquidacionTasaMenor);
	}
	...
	public String getNombreBean() {
		return "#{saic$grpTasaMenor$ABMLiquidacionTasaMenor$ABMLiquidacionTasaMenor}";
	}
