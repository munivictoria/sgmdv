 ...
 * 
 */
public class ABMLiquidacionOSP extends ABMPageBean {
	...
	private Label lblAlicuotasLiquidadas = new Label();

	private TextArea taAlicuotasLiquidadas = new TextArea();
	
	public Label getLblAlicuotasLiquidadas() {
		return lblAlicuotasLiquidadas;
	}

	public void setLblAlicuotasLiquidadas(Label lblAlicuotasLiquidadas) {
		this.lblAlicuotasLiquidadas = lblAlicuotasLiquidadas;
	}

	public TextArea getTaAlicuotasLiquidadas() {
		return taAlicuotasLiquidadas;
	}

	public void setTaAlicuotasLiquidadas(TextArea taALicuotasLiquidadas) {
		this.taAlicuotasLiquidadas = taALicuotasLiquidadas;
	}
	...
	protected void mostrarEstadoObjetosUsados() {
		LiquidacionTasa liquidacionOSP = (LiquidacionTasa) this.obtenerObjetoDelElementoPila(0);

		if(liquidacionOSP != null) {
			this.getTfPeriodo().setText(liquidacionOSP.getPeriodoAnio().toString());
			this.getTaFormula().setText(liquidacionOSP.getStringFormula());
			this.getTaParametros().setText(liquidacionOSP.getStringParametrosValuados());
			this.getTaParametrosValuadosAlicuota().setText(liquidacionOSP.getStringParametrosValuadosAlicuota());
			this.getTaAlicuotasLiquidadas().setText(liquidacionOSP.getStringAlicuotasLiquidadas());
			this.getTaModificadores().setText(liquidacionOSP.getStringModificadoresLiquidacion());
			this.getTaVencimientos().setText(liquidacionOSP.getStringVencimientos());
			if(liquidacionOSP.getRegistroExencionRegistroDeuda() != null) {
				this.getTaExencion().setText(liquidacionOSP.getRegistroExencionRegistroDeuda().toString());
			}

			this.getTfFechaObligacion().setText(liquidacionOSP.getFechaEmision());
			this.getTfPersona().setText(liquidacionOSP.getPersona());
			this.getTaObligacion().setText(liquidacionOSP.getStringObligacion());
			this.getTfCuotaLiquidada().setText(liquidacionOSP.getCuotaLiquidada());
			// this.getTfFechaVencimiento().setText(Conversor.getStringDeFechaCorta(liquidacionOSP.getFechaVencimiento()));
			// this.getTfTipoDeuda().setText(liquidacionOSP.getTipoDeuda().toString());
			this.getTfEstado().setText(liquidacionOSP.getEstado().toString());
			this.getTfMonto().setText(Conversor.getStringDeDouble(liquidacionOSP.getMonto()));
			if(liquidacionOSP.getFechaCancelacion() != null) {
				this.getTfFechaCancelacion().setText(liquidacionOSP.getFechaCancelacion());
			}
		}
	}
	...
	protected void procesarObjetoABM(Object pObject) {
		LiquidacionTasa liquidacionOSP = (LiquidacionTasa) pObject;
		try {
			this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().setLlave(this.getSessionBean1().getLlave());
			liquidacionOSP = (LiquidacionTasa) this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().getRegistroDeudaPorId(new Long(liquidacionOSP.getIdRegistroDeuda()));
		} catch(Exception e) {
			e.printStackTrace();
		}
		this.getElementoPila().getObjetos().set(0, liquidacionOSP);
	}
	...
	public String getNombreBean() {
		return "#{saic$grpOSP$ABMLiquidacionOSP$ABMLiquidacionOSP}";
	}
