 ...
 * 
 */
public class ABMLiquidacionSHPS extends ABMPageBean {
	...
	protected void mostrarEstadoObjetosUsados() {
		LiquidacionTasa liquidacionSHPS = (LiquidacionTasa) this.obtenerObjetoDelElementoPila(0);

		if(liquidacionSHPS != null) {
			this.getTfPeriodo().setText(liquidacionSHPS.getPeriodoAnio().toString());
			this.getTaFormula().setText(liquidacionSHPS.getStringFormula());
			this.getTaParametros().setText(liquidacionSHPS.getStringParametrosValuados());
			this.getTaParametrosValuadosAlicuota().setText(liquidacionSHPS.getStringParametrosValuadosAlicuota());

			this.getTaModificadores().setText(liquidacionSHPS.getStringModificadoresLiquidacion());
			this.getTaVencimientos().setText(liquidacionSHPS.getStringVencimientos());
			if(liquidacionSHPS.getRegistroExencionRegistroDeuda() != null) {
				this.getTaExencion().setText(liquidacionSHPS.getRegistroExencionRegistroDeuda().toString());
			}
			this.getTaObligacion().setText(liquidacionSHPS.getStringObligacion());
			this.getTfCuotaLiquidada().setText(liquidacionSHPS.getCuotaLiquidada());
			this.getTfEstado().setText(liquidacionSHPS.getEstado().toString());
			this.getTfMonto().setText(Conversor.getStringDeDouble(liquidacionSHPS.getMonto()));
			if(liquidacionSHPS.getFechaCancelacion() != null)
				this.getTfFechaCancelacion().setText(liquidacionSHPS.getFechaCancelacion());
		}
	}
	...
	protected void procesarObjetoABM(Object pObject) {
		LiquidacionTasa liquidacionSHPS = (LiquidacionTasa) pObject;
		try {
			this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().setLlave(this.getSessionBean1().getLlave());
			liquidacionSHPS = (LiquidacionTasa) this.getCommunicationSAICBean().getRemoteSystemLiquidacionTasa().getRegistroDeudaPorId(new Long(liquidacionSHPS.getIdRegistroDeuda()));
		} catch(Exception e) {
			e.printStackTrace();
		}
		this.getElementoPila().getObjetos().set(0, liquidacionSHPS);
	}
	...
	public String getNombreBean() {
		return "#{saic$grpSHPS$ABMLiquidacionSHPS$ABMLiquidacionSHPS}";
	}
