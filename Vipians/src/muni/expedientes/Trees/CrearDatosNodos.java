...
public class CrearDatosNodos {
	...
	public static TreeView.Datos crearDatos(Object pObject, int posicion) {
		...
		if (pObject instanceof Procedimiento) {
			...
			return new DatosNodo(p.getNombre(), "tnRaiz", URL_FASE,
					crearNodosDatos(p.getListaFasesProcedimiento()));
		}

		if (pObject instanceof Expediente) {
			Expediente e = (Expediente) pObject;
			return new DatosNodo(e.getAsunto(), "tnRaiz", URL_FASE,
					crearNodosDatos(e.getListaNodosExpedientes()));
		}

		if (pObject instanceof Fase) {
			Fase f = (Fase) pObject;
			return new DatosNodo(f.getPlantilla().toString(), F + posicion, URL_FASE,
					crearNodosDatos(f.getListaNodosExpedientes()));
		}
		if (pObject instanceof FaseProcedimiento) {
			FaseProcedimiento fp = (FaseProcedimiento) pObject;
			return new DatosNodo(fp.getFaseCatalogo().getNombre(), F + posicion, URL_FASE,
					crearNodosDatos(fp.getListaTramitesProcedimientos()));
		}

		if (pObject instanceof FaseCatalogo) {
			FaseCatalogo fp = (FaseCatalogo) pObject;
			return new DatosNodo(fp.getNombre(), F + posicion, URL_FASE,
					crearNodosDatos(fp.getListaTramitesCatalogos()));
		}
