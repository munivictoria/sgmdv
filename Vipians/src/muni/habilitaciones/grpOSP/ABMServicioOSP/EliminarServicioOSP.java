 ...
 */
public class EliminarServicioOSP extends AbstractPageBean {
    ...
    private final String NOMBRE_PAGINA = "Eliminar C\363digo de Servicio de OSP";
    // nombre del caso de navegacion para llegar a esta pagina.
    private final String CASO_NAVEGACION = "EliminarServicioOSP";
    ...
    public void init() {
        ...
        try {
            ...
            log("EliminarServicioOSP Initialization Failure", e);
...
//            this.cargarValoresPorDefecto();
    ...
    private void guardarEstadoObjetosUsados() {
        ...
        Object unidad = ddUnidadMedida.getSelected();
        UnidadMedida unidadMedida = null;
        if ((unidad != null) && (unidad.toString().length() > 0)) unidadMedida = UnidadMedida.valueOf(unidad.toString());

        Object baseConsumo = this.getTfBaseConsumo().getText();
        Object valorPorExcedente = this.getTfValorPorExcedente().getText();
        
        if (codigo != null && codigo != "") servicioOSP.setCodigo(codigo.toString()); else servicioOSP.setCodigo(null);
        if (nombre != null && nombre != "") servicioOSP.setNombre(nombre.toString()); else servicioOSP.setNombre(null);
        if (valor != null && valor != "") servicioOSP.setValor(Conversor.getDoubleDeString(valor.toString())); else servicioOSP.setValor(null);
        
        if (medido != null && medido != "") servicioOSP.setMedido(((Boolean)medido).booleanValue()); else servicioOSP.setMedido(false);
        if (servicioOSP.isMedido()) {
            if (unidadMedida != null) servicioOSP.setUnidadMedida(unidadMedida); else servicioOSP.setUnidadMedida(null);
            if (baseConsumo != null && baseConsumo != "") servicioOSP.setBaseConsumo(Conversor.getDoubleDeString(baseConsumo.toString())); else servicioOSP.setBaseConsumo(null);
            if (valorPorExcedente != null && valorPorExcedente != "") servicioOSP.setValorPorExcedente(Conversor.getDoubleDeString(valorPorExcedente.toString())); else servicioOSP.setValorPorExcedente(null);
        } else {
    ...
    private void mostrarEstadoObjetosUsados() {
        ...
        if (servicioOSP.getValor() != null) this.getTfValor().setText(Conversor.getStringDeDouble(servicioOSP.getValor()));
        ...
        if (servicioOSP.isMedido()) {
            if (servicioOSP.getBaseConsumo() != null) this.getTfBaseConsumo().setText(Conversor.getStringDeDouble(servicioOSP.getBaseConsumo()));
            if (servicioOSP.getValorPorExcedente() != null) this.getTfValorPorExcedente().setText(Conversor.getStringDeDouble(servicioOSP.getValorPorExcedente()));
            ddUnidadMedida.setSelected(Util.getEnumNameFromString(servicioOSP.getUnidadMedida().toString()));
            ddUnidadMedidaDefaultOptions.setSelectedValue(Util.getEnumNameFromString(servicioOSP.getUnidadMedida().toString()));
        } else {
            this.getTfBaseConsumo().setText(null);
            this.getTfValorPorExcedente().setText(null);
            ddUnidadMedida.setSelected(null);
            ddUnidadMedidaDefaultOptions.setSelectedValue(null);
        }
    ...
    public String btnEliminar_action() {
        ...
        if (ultimo) {
            ...
            try {
                ...
                this.getCommunicationHabilitacionesBean().getRemoteSystemDocumentoOSP().deleteServicioOSP(servicioOSP);
                
                this.getRequestBean1().setRespuestaABM(servicioOSP);
                
                info("El C\363digo de Servicio de OySP se elimin\363 exitosamente.");
