 ...
 */
public class ModificarDocEspTasaMenor extends AbstractPageBean {
    ...
    private final String NOMBRE_PAGINA = "Modificar Obligaci\363n: Tasa Menor";
    // nombre del caso de navegacion para llegar a esta pagina.
    private final String CASO_NAVEGACION = "ModificarDocEspTasaMenor";
     ...
     */
    public ModificarDocEspTasaMenor() {
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("ModificarDocEspTasaMenor Initialization Failure", e);
...
//        }
        ...
        if (this.getRequestBean1().getObjetoABM() != null) {
            ...
            try {
                if (documentoTasaMenor.getParcela() != null) {
            ...
            if (documentoTasaMenor.getListaAtributosDinamicos() != null) {
                try {
//                    atributosDinamicos = (ArrayList) this.getComunicationBean().getRemoteSystemParametro().getAtributosPorRecurso(DocumentoTGI.serialVersionUID, documentoTGI.getListaAtributosDinamicos());
        ...
        atributosDinamicos = (ArrayList) this.obtenerObjetoDelElementoPila(5, ArrayList.class);
    ...
    public String btnSeleccionarDomicilioPostal_action() {
        ...
        if (ultimo) {
            ...
            Persona locPersona = (Persona) this.obtenerObjetoDelElementoPila(2, Persona.class);
            Parcela locParcela = (Parcela) this.obtenerObjetoDelElementoPila(3, Parcela.class);
            Domicilio locDomicilio = (Domicilio) this.obtenerObjetoDelElementoPila(posicionObjetoSeleccionado, Domicilio.class);

            if (locDomicilio.getIdDomicilio() == locParcela.getDomicilioParcelario().getIdDomicilio() || locDomicilio.getIdDomicilio() == locPersona.getDomicilioPostal().getIdDomicilio()) {
    ...
    public String btnGuardar_action() {
        ...
        if (ultimo) {
            ...
            try {
                ...
                DocumentoTasaMenor documentoTasaMenor = (DocumentoTasaMenor) this.getElementoPila().getObjetos().get(1);

                if (!v.fechaNoMayorAFechaActual(documentoTasaMenor.getFechaInicioActividad(), "Inicio de Actividades")) {
                ...
                if (v2.getErrores().size() > 0) {
                    ...
                    for (int i = 0; i < v2.getErrores().size(); i++) {
                        warn(v2.getErrores().toArray()[i].toString());
                    }
                    return null;
                }

                documentoTasaMenor.setListaAtributosDinamicos(atributosDinamicos);

                // System.out.println("id dom: " + documentoTGI.getDomicilio().getIdDomicilio());
                this.getCommunicationHabilitacionesBean().getRemoteSystemDocumentoTasaMenor().setLlave(this.getSessionBean1().getLlave());
                this.getCommunicationHabilitacionesBean().getRemoteSystemDocumentoTasaMenor().updateDocumentoTasaMenor(documentoTasaMenor);

                info("La Obligaci\363n de Tasa Menor se modific\363 exitosamente.");
