/*
 * AgregarPlantillaPermiso.java
 *
 * Created on 17 de noviembre de 2006, 10:27
 * Copyright Trascender
 */
package muni.habilitaciones.ABMPlantillaPermiso;
 ...
 */
public class AgregarPlantillaPermiso extends AbstractPageBean {
    ...
    private TextField tfPadre = new TextField();

    public TextField getTfPadre() {
        return tfPadre;
    }

    public void setTfPadre(TextField tf) {
        this.tfPadre = tf;
    }
    ...
    private TextField tfPosibleAutorizador = new TextField();

    public TextField getTfPosibleAutorizador() {
        return tfPosibleAutorizador;
    }

    public void setTfPosibleAutorizador(TextField tf) {
        this.tfPosibleAutorizador = tf;
    }
    ...
    public void setLdpRoles(ObjectListDataProvider oldp) {
        this.ldpRoles = oldp;
    }
    ...
    // CAMBIAR: Vicular a los campos hidden.
    ...
    private final String NOMBRE_PAGINA = "Agregar Permiso";
    // nombre del caso de navegacion para llegar a esta pagina.
    private final String CASO_NAVEGACION = "AgregarPlantillaPermiso";
     ...
     */
    public AgregarPlantillaPermiso() {
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("AgregarPlantillaPermiso Initialization Failure", e);
    ...
    private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
        ...
        ep.getObjetos().add(ind++, new PlantillaPermiso());
        ep.getObjetos().add(ind++, new String()); // nombre del padre
    ...
    private void guardarEstadoObjetosUsados() {
        ...
        PlantillaPermiso permiso = (PlantillaPermiso) this.obtenerObjetoDelElementoPila(0, PlantillaPermiso.class);
        Rol rol = (Rol) this.obtenerObjetoDelElementoPila(2, Rol.class);
        
        Object nombre = this.getTfNombre().getText();
        Object autoriz = this.getTfPosibleAutorizador().getText();
        
        if (nombre != null && nombre != "") permiso.setNombre(nombre.toString()); else permiso.setNombre(null);
        if (autoriz != null && autoriz != "") permiso.setPosibleAutorizador(autoriz.toString()); else permiso.setPosibleAutorizador(null);

        if (rol.getIdRol()==-1) rol = null;
        permiso.setRol(rol);
        
        this.getElementoPila().getObjetos().set(0, permiso);
        this.getElementoPila().getObjetos().set(2, rol);
    }
    
    private void mostrarEstadoObjetosUsados() {
        // CAMBIAR: Crear una instancia por cada objeto manejado en la pagina
        PlantillaPermiso permiso = null;
        PlantillaPermiso permisoPadre = null;
        String padre = null;
        Rol rol = null;
        
        // CAMBIAR: Obtener datos del Request y asignarlos a los textField
        if (this.getRequestBean1().getObjetoABM() != null) {
            permisoPadre = (PlantillaPermiso) this.getRequestBean1().getObjetoABM();

            permiso= (PlantillaPermiso) this.obtenerObjetoDelElementoPila(0, PlantillaPermiso.class);

            this.getElementoPila().getObjetos().set(0, permiso);
            this.getElementoPila().getObjetos().set(1, permisoPadre.getNombre());//new
        }
        
        if (this.getRequestBean1().getObjetoSeleccion() != null) {
            Object selec = this.getRequestBean1().getObjetoSeleccion();
            
            if (selec instanceof Rol) {
                rol = (Rol) selec;
                this.getElementoPila().getObjetos().set(2, selec);
            }
            else {//nombre del padre
                this.getElementoPila().getObjetos().set(1, selec);
            }
        }
        
        permiso = (PlantillaPermiso) this.obtenerObjetoDelElementoPila(0, PlantillaPermiso.class);
        padre =  (String) this.obtenerObjetoDelElementoPila(1, String.class);
        rol = (Rol) this.obtenerObjetoDelElementoPila(2, Rol.class);
        
        this.getTfPadre().setText(padre);
        this.getTfNombre().setText(permiso.getNombre());
        this.getTfPosibleAutorizador().setText(permiso.getPosibleAutorizador());
    ...
    public String btnGuardar_action() {
        ...
        if (ultimo) {
            ...
            try {
                ...
                // CAMBIAR:
                Object permiso = this.getElementoPila().getObjetos().get(0);
                this.getRequestBean1().setRespuestaABM(permiso);
                
                info("Se agreg\363 un Permiso.");
                retorno = this.prepararParaVolver(Constantes.ACCION_AGREGAR);
            } catch (Exception ex) {
                if (ex instanceof TrascenderException) {
                    if (((TrascenderException)ex).getCodeTrascenderException()<3300) retorno = null;
    ...
    public String btnLimpiarRol_action() {
        ...
        if(ultimo) {
            ...
            this.limpiarObjeto(2, Rol.class, this.getTfRol());
            this.guardarEstadoObjetosUsados();
        } else {
            retorno = this.prepararCaducidad();
        }
