 ...
 */
public class AdminDocEspTGI extends AdminPageBean {
	...
	protected void _init() throws Exception {
		...
		Set<String> locListaPlantillasObligaciones = getApplicationBean1().getMapaPlantillaObligacionTGI().keySet();
	...
	private ObjectListDataProvider ldpDocEspTGI = new ObjectListDataProvider();

	public ObjectListDataProvider getLdpDocEspTGI() {
		return ldpDocEspTGI;
	}

	public void setLdpDocEspTGI(ObjectListDataProvider oldp) {
		this.ldpDocEspTGI = oldp;
	}
	...
	protected ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		...
		FiltroObligacionTGI locFiltro = this.getFiltro();
		List atributosDinamicos = null;
		try {
			atributosDinamicos = this.getComunicationBean().getRemoteSystemParametro().getAtributosPorRecurso(DocumentoTGI.serialVersionUID, null, true);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private PlantillaObligacion getPlantillaPorNombre(String pPlantilla){
		return getApplicationBean1().getMapaPlantillaObligacionTGI().get(pPlantilla);
	}
	...
	protected void mostrarEstadoObjetosUsados() {
		...
		panelAtributoDinamico = new PanelAtributoDinamico(locFiltro.getListaAtributosDinamicos(), "#{habilitaciones$grpTGI$ABMDocEspTGI$AdminDocEspTGI}");
		...
		if(locFiltro.getPoseeExenciones() != null){
			this.getDdExenciones().setSelected(locFiltro.getPoseeExenciones() == true? "Posee":"No posee");
		}
	...
	public ObjectListDataProvider getObjectListDataProvider() {
		...
		return this.getLdpDocEspTGI();
	}

	@Override
	public ArrayList getListaDelCommunication() {
		// CAMBIAR: Utilizar la Lista del Comunication que corresponda
		return this.getCommunicationHabilitacionesBean().getListaDocEspTGI();
	}
	// </editor-fold>
	...
	//                error(NOMBRE_PAGINA + " - Seleccionar Plantilla de Obligaci\363n: " + ex.getMessage());
	//            }
	//
	//            this.getRequestBean1().setObjetoSeleccion(this.getCommunicationHabilitacionesBean().getMapaTipoTasa().get("TGI"));
	//
	...
	public String btnAgregar_action() {
		...
		nomNoVacios[pos++] = "Plantilla de Obligaci\363n del Tipo TGI";
		...
		return toAbm(new DocEspTGIModel().new AgregarDocEspTGIController());
	}

	public String btnModificar_action() {
		return toAbm(new DocEspTGIModel().new ModificarDocEspTGIController());
	}
		...
		if (ultimo) {
			...
			try {
				...
				if (rk != null) {
					...
					if (documentoTGI.getParcela().getTituloPropiedad() == null) {
	...
	public String btnEliminar_action() {
		return toAbm(new DocEspTGIModel(). new EliminarDocEspTGIController());
	}
	...
	public String btnConsultar_action() {
		return toAbm(new DocEspTGIModel().new ConsultarDocEspController());
	}
	...
	protected void setListaDelCommunication(List lista) {
		this.getCommunicationHabilitacionesBean().setListaDocEspTGI((ArrayList)lista);
	}
	...
	protected FiltroAbstracto buscar(FiltroAbstracto pFiltro) throws Exception {
		FiltroObligacionTGI locFiltroObligacionTGI = (FiltroObligacionTGI) pFiltro;
		return this.getCommunicationHabilitacionesBean().getRemoteSystemObligacion().findListaObligacionesTGI(locFiltroObligacionTGI);
	}

	@Override
	protected String getNombrePagina() {
		return "Administraci\363n de Obligaciones TGI";
	}

	@Override
	protected String getCasoNavegacion() {
		return "AdminDocEspTGI";
	}
	@Override
	public PaginatedTable getPaginatedTable(){
		return this.getCommunicationHabilitacionesBean().getTablaDocumentoTGI();
	}
		...
		if (ultimo) {
			try {
				...
				if (rk != null) {
					...
					DocumentoTGI docTgi = (DocumentoTGI) locObligacion.getDocumentoEspecializado();

					this.getCommunicationSAICBean().getRemoteSystemImpresion().setLlave(this.getSessionBean1().getLlave());

					//					JasperPrint jp = this.getCommunicationSAICBean().getRemoteSystemImpresion().getReporteLiquidacionDeudaServiciosMunicipales(docTgi);
	...
	public String getNombreBean(){
		return "#{habilitaciones$grpTGI$ABMDocEspTGI$AdminDocEspTGI}";
	}
