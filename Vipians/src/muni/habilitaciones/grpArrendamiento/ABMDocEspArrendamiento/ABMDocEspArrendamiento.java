 ...
 * </p>
 */
public class ABMDocEspArrendamiento extends ABMPageBean {
	...
	protected ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		...
		ep.getObjetos().add(ind++, new DocumentoArrendamiento());
	...
	protected void guardarEstadoObjetosUsados() {
		...
		DocumentoArrendamiento documentoArrendamiento = this.obtenerObjetoDelElementoPila(ind++, DocumentoArrendamiento.class);
		...
		documentoArrendamiento.setNombre(getTextAreaValue(this.getTaNombre()));
		documentoArrendamiento.setFechaInicioActividad(getTextFieldValueDate(this.getTfFechaInicio()));
		documentoArrendamiento.setFechaCeseActividad(getTextFieldValueDate(this.getTfFechaCese()));

		if(parcela.getIdParcela() == -1) {
			parcela = null;
		} else {
			documentoArrendamiento.setParcela(parcela);
		...
		documentoArrendamiento.setDomicilio(domicilio);
		...
		obligacion.setDocumentoEspecializado(documentoArrendamiento);
		...
		documentoArrendamiento.setListaAtributosDinamicos(atributosDinamicos);
		...
		this.getElementoPila().getObjetos().set(ind++, documentoArrendamiento);
	...
	protected void mostrarEstadoObjetosUsados() {
		...
		try {
			DocumentoArrendamiento locDocumento = (DocumentoArrendamiento) this.obtenerObjetoDelElementoPila(1);
			this.getElementoPila()
					.getObjetos()
					.set(5, this.getComunicationBean().getRemoteSystemParametro().getAtributosPorRecurso(DocumentoArrendamiento.serialVersionUID, locDocumento.getListaAtributosDinamicos(), null));
		} catch(Exception e) {
			e.printStackTrace();
		}
		...
		ind = 0;
		ind++;
		documentoArrendamiento = this.obtenerObjetoDelElementoPila(ind++, DocumentoArrendamiento.class);
		...
		panelAtributoDinamico = new PanelAtributoDinamico(atributosDinamicos, "#{habilitaciones$grpArrendamiento$ABMDocEspArrendamiento$ABMDocEspArrendamiento}");
		...
		this.getTaNombre().setText(documentoArrendamiento.getNombre());
		if(documentoArrendamiento.getFechaInicioActividad() != null) {
			this.getTfFechaInicio().setText(Conversor.getStringDeFechaCorta(documentoArrendamiento.getFechaInicioActividad()));
		}
		if(documentoArrendamiento.getFechaCeseActividad() != null) {
			this.getTfFechaCese().setText(Conversor.getStringDeFechaCorta(documentoArrendamiento.getFechaCeseActividad()));
		}
		if(persona != null) {
	...
	private void setListaDelCommunicationAtributosDinamicos(List lista) {
		...
		this.getCommunicationHabilitacionesBean().setListaAtributosDinamicosArrendamiento(lista);
	}
	...
	protected void procesarObjetoSeleccion(Object pObject) {
		if(pObject instanceof Parcela) {
			...
			try {
				...
				DocumentoArrendamiento documentoArrendamiento = this.obtenerObjetoDelElementoPila(1, DocumentoArrendamiento.class);
				documentoArrendamiento.setParcela(locParcela);
				...
				this.getElementoPila().getObjetos().set(1, documentoArrendamiento);
	...
	protected void procesarObjetoABM(Object pObject) {
		...
		if(pObject instanceof PlantillaObligacion) {
			...
			documentoArrendamiento = (DocumentoArrendamiento) obligacion.getDocumentoEspecializado();
			...
			persona = documentoArrendamiento.getObligacion().getPersona();

			try {
				this.getComunicationCatastroBean().getRemoteSystemInformacionParcelaria().setLlave(this.getSessionBean1().getLlave());
				parcela = this.getComunicationCatastroBean().getRemoteSystemInformacionParcelaria().getParcelaPorId(documentoArrendamiento.getParcela().getIdParcela());
			} catch(Exception ex) {
			...
			domicilio = documentoArrendamiento.getDomicilio();

			if(documentoArrendamiento.getListaAtributosDinamicos() != null) {
				try {
					atributosDinamicos = (ArrayList) this.getComunicationBean().getRemoteSystemParametro()
							.getAtributosPorRecurso(DocumentoArrendamiento.serialVersionUID, documentoArrendamiento.getListaAtributosDinamicos(), null);
				} catch(Exception ex) {
					ex.printStackTrace();
				}
			}
	...
	protected void _prerender() throws Exception {
		...
		DocumentoArrendamiento locDocumento = this.obtenerObjetoDelElementoPila(1, DocumentoArrendamiento.class);
	...
	public String getNombreBean() {
		return "#{habilitaciones$grpArrendamiento$ABMDocEspArrendamiento$ABMDocEspArrendamiento}";
	}
