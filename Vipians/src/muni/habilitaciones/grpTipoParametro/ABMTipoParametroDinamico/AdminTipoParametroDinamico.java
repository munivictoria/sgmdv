/*
 * AdminTipoParametroDinamico.java
 *
 * Created on 18 de octubre de 2006, 8:55
 * Copyright Trascender SRL
 */

package muni.habilitaciones.grpTipoParametro.ABMTipoParametroDinamico;
 ...
 * </p>
 */
public class AdminTipoParametroDinamico extends AbstractPageBean {
	...
	private String NOMBRE_PAGINA = "Administraci\363n de Par\341metros Din\341micos";
	private final String CASO_NAVEGACION = "AdminTipoParametroDinamico";
	...
	private void _init() throws Exception {
		if(this.getRequestBean1().getObjetoSeleccion() != null) {
			this.getHead1().setTitle("Administraci\363n de Par\341metros de " + ((TipoObligacion) this.getRequestBean1().getObjetoSeleccion()).getNombre());
		}
		if(this.getListaDelCommunication() != null) {
			this.getObjectListDataProvider().setList(this.getListaDelCommunication());
			System.out.println("comunication != null");
		}
	...
	private ObjectListDataProvider ldpTipoParametroDinamico = new ObjectListDataProvider();

	public ObjectListDataProvider getLdpTipoParametroDinamico() {
		return ldpTipoParametroDinamico;
	}

	public void setLdpTipoParametroDinamico(ObjectListDataProvider ldpTipoParametroDinamico) {
		this.ldpTipoParametroDinamico = ldpTipoParametroDinamico;
	}
	...
	private TableSelectPhaseListener getTableSelectPhaseListener() {
		return this.getCommunicationHabilitacionesBean().getTablePhaseListenerParametroDinamico();
	}
	 ...
	 * </p>
	 */
	public AdminTipoParametroDinamico() {
	}
	...
	public void init() {
		...
		try {
			...
			log("AdminTipoParametroDinamico Initialization Failure", e);
	...
	/**
	 * <p>
	 * Método de devolución de llamada al que se llama cuando el árbol de componentes se ha restaurado, pero antes de que se produzca el procesamiento de
	 * cualquier evento. Este método <strong>sólo</strong> se llamará en una petición de devolución de envío que esté procesando el envío de un formulario.
	 * Puede personalizar este método para asignar recursos necesarios para los controladores de eventos.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Método de devolución de llamada al que se llama justo antes del procesamiento. <strong>Sólo</strong> se llamará a este método en la página que se
	 * procesa, no se llamará, por ejemplo, en una página que ha procesado una devolución de envío y a continuación ha navegado hasta otra página. Puede
	 * personalizar este método para asignar recursos necesarios para procesar esta página.
	 * </p>
	 */
	...
	private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		int ind = 0;
		ep.getObjetos().add(ind++, (TipoObligacion) this.getRequestBean1().getObjetoSeleccion());
	...
	private void guardarEstadoObjetosUsados() {
		...
		TipoObligacion tipoObligacion = (TipoObligacion) this.obtenerObjetoDelElementoPila(0, TipoObligacion.class);

		this.getElementoPila().getObjetos().set(0, tipoObligacion);
	}
...
//		if(this.getRequestBean1().getObjetoSeleccion() != null) {
//			tipoObligacion = (TipoObligacion) this.getRequestBean1().getObjetoSeleccion();
//			this.getRequestBean1().setObjetoSeleccion(null);
//			this.getElementoPila().getObjetos().set(0, tipoObligacion);
//		}

		if(this.getLdpTipoParametroDinamico().getList() != null) {
...
//			this.refrescarTabla();
//		} catch(Exception ex) {
//			Logger.getLogger(AdminTipoParametroDinamico.class.getName()).log(Level.SEVERE, null, ex);
//		}
	}
	...
	private void refrescarTabla() throws Exception {
		...
		if(tipoObligacion != null) {
			this.setListaDelCommunication((ArrayList) this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaParametrosDinamicos(tipoObligacion));
	...
	private ObjectListDataProvider getObjectListDataProvider() {
		return this.getLdpTipoParametroDinamico();
	}

	private ArrayList getListaDelCommunication() {
		return this.getCommunicationHabilitacionesBean().getListaTiposParametroDinamico();
	}

	private void setListaDelCommunication(ArrayList lista) {
		this.getCommunicationHabilitacionesBean().setListaTiposParametroDinamico(lista);
	}
	...
	public String btnSeleccionar_action() {
		...
		boolean ultimo = this.ultimoElementoPilaDeSubSesion();
		if(ultimo) {
			TipoParametro locTipoParametroDinamico = null;
			...
			try {
				...
				for(int i = 0; i < selectedRowKeys.length; i++) {
					...
					locTipoParametroDinamico = (TipoParametro) obj;
					this.getRequestBean1().getObjetosSeleccionMultiple().add(locTipoParametroDinamico);
				}
			} catch(Exception ex) {
	...
	// this.getElementoPila().setPosicionGlobal(pos.longValue());
	//
	// if (rk != null) retorno = lnkModificar;
	// } else {
	// retorno = this.prepararCaducidad();
	// }
	// return retorno;
	...
	public String btnExportar_action() {
		...
		if(ultimo) {
			try {
				JasperPrint jp = ImpresionReporteDinamico.imprimirLista(this.getListaDelCommunication(), this.getTableRowGroup1(), "Reporte Din\341mico de Par\341metros Din\341micos");
				...
				FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("reportName", "Reporte_ParametrosDinamicos");
