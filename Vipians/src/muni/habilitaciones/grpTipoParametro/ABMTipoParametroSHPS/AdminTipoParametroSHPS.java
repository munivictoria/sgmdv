/*
 * AdminTipoParametroSHPS.java
 *
 * Created on 18 de octubre de 2006, 8:55
 * Copyright Trascender SRL
 */
package muni.habilitaciones.grpTipoParametro.ABMTipoParametroSHPS;
 ...
 */
public class AdminTipoParametroSHPS extends AbstractPageBean {
	...
	private final String NOMBRE_PAGINA = "Administraci\363n de Par\341metros de SHPS";
	// nombre del caso de navegacion para llegar a esta pagina.
	private final String CASO_NAVEGACION = "AdminTipoParametroSHPS";
	...
	private final String lnkAgregar = "AgregarTipoParametroSHPS";
	private final String lnkModificar = "ModificarTipoParametroSHPS";
	private final String lnkEliminar = "EliminarTipoParametroSHPS";
	// </editor-fold>
	...
	private ObjectListDataProvider ldpTipoParametroSHPS = new ObjectListDataProvider();

	public ObjectListDataProvider getLdpTipoParametroSHPS() {
		return ldpTipoParametroSHPS;
	}

	public void setLdpTipoParametroSHPS(ObjectListDataProvider oldp) {
		this.ldpTipoParametroSHPS = oldp;
	}
		...
		if (existeIdSubSesionReq && existeIdPaginaReq && !existeIdSubSesionPag && !existeIdPaginaPag) {
			...
			if (this.getListaDelCommunication() != null
					...
					&& (this.getRequestBean1().getAccion().equals(Constantes.ACCION_AGREGAR)
							...
							|| this.getRequestBean1().getAccion().equals(Constantes.ACCION_SELECCIONAR))) {
				try {
					this.refrescarTabla();
				} catch (Exception ex) {
					this.limpiarTabla();
	...
	private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
		...
		try {
			//ep.getObjetos().add(ind++, new TipoParametroSHPS());
	...
	private void guardarEstadoObjetosUsados() {
		...
		String tipoParametroSHPS = (String) this.obtenerObjetoDelElementoPila(0, String.class);
		if (this.getRequestBean1().getObjetoSeleccion() != null) {
			String seleccionado = (String) this.getRequestBean1().getObjetoSeleccion();
			System.out.println("STRING TIPO PARAMETROOOOO1 ---" + seleccionado);
			this.getElementoPila().getObjetos().set(0, seleccionado);
		}
		this.getElementoPila().getObjetos().set(0, tipoParametroSHPS);
	}

	private void mostrarEstadoObjetosUsados() {

		// CAMBIAR: Revisar el metodo completo.
		String tipoParametroSHPS = null;

		tipoParametroSHPS = (String) this.obtenerObjetoDelElementoPila(0, String.class);
		if (this.getRequestBean1().getObjetoSeleccion() != null) {
			String seleccionado = (String) this.getRequestBean1().getObjetoSeleccion();
			System.out.println("STRING TIPO PARAMETROOOOO1 ---" + seleccionado);
			this.getElementoPila().getObjetos().set(0, seleccionado);
		...
		if (this.getLdpTipoParametroSHPS().getList() != null){
		...
		try {
			...
			Logger.getLogger(AdminTipoParametroSHPS.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
	...
	private void refrescarTabla() throws Exception {
		...
		String tipoParametroSHPS = (String) this.obtenerObjetoDelElementoPila(0, String.class);
		System.out.println("STRING TIPO PARAMETROOOOO2 ---" + tipoParametroSHPS);
		...
		if (tipoParametroSHPS != null && tipoParametroSHPS != "") {
			if (tipoParametroSHPS.equals("SHPS")) {
				System.out.println("shps----");
				this.setListaDelCommunication(this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaTiposParametrosSHPS());
			} else if (tipoParametroSHPS.equals("ALICUOTA_SHPS")) {
				System.out.println("alicuota----");
				this.setListaDelCommunication(this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaParametrosAlicuotaSHPS());
			}
			this.getObjectListDataProvider().setList(this.getListaDelCommunication());
		}       
	...
	private ObjectListDataProvider getObjectListDataProvider() {
		...
		return this.getLdpTipoParametroSHPS();
	}

	private List getListaDelCommunication() {
		// CAMBIAR: Utilizar la Lista del Comunication que corresponda
		return this.getCommunicationHabilitacionesBean().getListaTiposParametroSHPS();
	}

	private void setListaDelCommunication(List lista) {
		// CAMBIAR: Utilizar la Lista del Comunication que corresponda
		this.getCommunicationHabilitacionesBean().setListaTiposParametroSHPS(lista);
	}
	...
	private TableSelectPhaseListener getTableSelectPhaseListener() {
		...
		return this.getCommunicationHabilitacionesBean().getTablePhaseListenerParametroSHPS();
	}
	...
	public String btnSeleccionar_action() {
		...
		boolean ultimo = this.ultimoElementoPilaDeSubSesion();
		if (ultimo) {
			Object locTipoParametroSHPS = null;
			...
			try {
				...
				for (int i = 0; i < selectedRowKeys.length; i++) {
					...
					if (tipoSeleccionado != null && tipoSeleccionado != "") {
						if (tipoSeleccionado.equals("SHPS")) {
							locTipoParametroSHPS = obj;
						} else if (tipoSeleccionado.equals("ALICUOTA_SHPS")) {
							locTipoParametroSHPS = obj;
						}
						this.getRequestBean1().getObjetosSeleccionMultiple().add(locTipoParametroSHPS);
					}
				}
			} catch (Exception ex) {
	...
	public String btnExportar_action() {
		...
		if (ultimo) {
			try {
				JasperPrint jp = ImpresionReporteDinamico.imprimirLista(this.getListaDelCommunication(), this.getTableRowGroup1(), "Reporte Din\341mico de Par\341metros SHPS");
				...
				FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("reportName", "Reporte_ParametrosSHPS");
