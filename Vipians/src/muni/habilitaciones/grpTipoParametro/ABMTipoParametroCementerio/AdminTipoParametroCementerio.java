package muni.habilitaciones.grpTipoParametro.ABMTipoParametroCementerio;
...
public class AdminTipoParametroCementerio extends AbstractPageBean {
    ...
    private final String NOMBRE_PAGINA = "Administraci\363n de Par\341metros de Cementerio";
    private final String CASO_NAVEGACION = "AdminTipoParametroCementerio";
    private final String CASO_NAV_CADUCIDAD = "PaginaCaducidad";
    private final String CASO_NAV_POST_CADUCIDAD = "Main";
    private final boolean PUEDE_SER_PAGINA_INICIAL = true;
    private final String lnkAgregar = "AgregarTipoParametroCementerio";
    private final String lnkModificar = "ModificarTipoParametroCementerio";
    private final String lnkEliminar = "EliminarTipoParametroCementerio";
    ...
    private ObjectListDataProvider ldpTipoParametroCementerio = new ObjectListDataProvider();
    
    public ObjectListDataProvider getLdpTipoParametroCementerio() {
		return ldpTipoParametroCementerio;
	}

	public void setLdpTipoParametroCementerio(ObjectListDataProvider ldpTipoParametroCementerio) {
		this.ldpTipoParametroCementerio = ldpTipoParametroCementerio;
	}
...
protected HtmlAjaxCommandButton btnReiniciar = new HtmlAjaxCommandButton();
    ...
    private TableSelectPhaseListener getTableSelectPhaseListener() {
        return this.getCommunicationHabilitacionesBean().getTablePhaseListenerParametroCementerio();
    }

    public AdminTipoParametroCementerio() {
    }
    ...
    public void init() {
        ...
        try {
            ...
            log("AdminTipoParametroCementerio Initialization Failure", e);
    ...
    private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
        int ind = 0;
        ep.getObjetos().add(ind++, new String()); // para saber si es TipoParametroCementerio/TipoParametroParcelaCementerio/TipoParametroTipoSepultura

        return ep;
    }

    private void guardarEstadoObjetosUsados() {
    	String tipoParametroCementerio = (String) this.obtenerObjetoDelElementoPila(0, String.class);

        this.getElementoPila().getObjetos().set(0, tipoParametroCementerio);
    }
    ...
    private void mostrarEstadoObjetosUsados() {
    	...
    	if (this.getRequestBean1().getObjetoSeleccion() != null) {
    		tipoParametroCementerio = (String) this.getRequestBean1().getObjetoSeleccion().toString();
    		System.out.println("OBJETO SELECCION: " + tipoParametroCementerio);
            this.getElementoPila().getObjetos().set(0, tipoParametroCementerio);
        }
    	
        if (this.getLdpTipoParametroCementerio().getList() != null){
        ...
        try {
            ...
            Logger.getLogger(AdminTipoParametroCementerio.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void refrescarTabla() throws Exception {
    	...
    	if (tipoParametroCementerio != null && tipoParametroCementerio != "") {
    		
    		if(tipoParametroCementerio.equals("CEMENTERIO")){
    			this.setListaDelCommunication((ArrayList) this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaTiposParametrosCementerio());
    		} else if(tipoParametroCementerio.equals("PARCELA_CEMENTERIO")){
    			this.setListaDelCommunication((ArrayList) this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaTiposParametrosParcelaCementerio());
    		} else if(tipoParametroCementerio.equals("TIPO_SEPULTURA")){
    			this.setListaDelCommunication((ArrayList) this.getCommunicationHabilitacionesBean().getRemoteSystemTipoTasa().getListaTiposParametrosTipoSepultura());
    		}
    ...
    private ObjectListDataProvider getObjectListDataProvider() {
        ...
        return this.getLdpTipoParametroCementerio();
    }

    private List getListaDelCommunication() {
        return this.getCommunicationHabilitacionesBean().getListaTiposParametroCementerio();
    }

    private void setListaDelCommunication(List lista) {
        this.getCommunicationHabilitacionesBean().setListaTiposParametroCementerio(lista);
    }
    ...
    public String btnSeleccionar_action() {
        ...
        boolean ultimo = this.ultimoElementoPilaDeSubSesion();
        if (ultimo) {
        	Object locTipoParametroCementerio = null;
            ...
            try {
                ...
                for (int i = 0; i < selectedRowKeys.length; i++) {
                    ...
                    if (tipoSeleccionado != null && tipoSeleccionado != "") {
                        if (tipoSeleccionado.equals("CEMENTERIO")) {
                        	locTipoParametroCementerio = (TipoParametro) obj;
                        } else if (tipoSeleccionado.equals("PARCELA_CEMENTERIO") || tipoSeleccionado.equals("TIPO_SEPULTURA")) {
                        	locTipoParametroCementerio = (TipoParametroAlicuota) obj;
                        }
                        this.getRequestBean1().getObjetosSeleccionMultiple().add(locTipoParametroCementerio);
                    }
                }
            } catch (Exception ex) {
    ...
    public String btnExportar_action() {
        ...
        if (ultimo) {
            try {
                JasperPrint jp = ImpresionReporteDinamico.imprimirLista(this.getListaDelCommunication(), this.getTableRowGroup1(), "Reporte Din\341mico de Par\341metros Cementerio");
                ...
                FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("reportName", "Reporte_ParametrosCementerio");
