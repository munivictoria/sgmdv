 ...
 */
public class ModificarDomicilio extends AbstractPageBean {
    ...
    private final String NOMBRE_PAGINA = "Modificar Domicilio";
    // nombre del caso de navegacion para llegar a esta pagina.
    private final String CASO_NAVEGACION = "ModificarDomicilio";
...
//        this.cargarComboCallesFinalizaCuadras(null);
    ...
    public void init() {
        ...
        ddCalleOptions.setOptions(opCalles);
            ...
            log("ModificarDomicilio Initialization Failure", e);
    ...
    private ElementoPila agregarObjetosAElementoPila(ElementoPila ep) {
     ...
     private Calle getCallePorNombre(String pCalle){
        System.out.println("todas las calles--- " + this.getComunicationCatastroBean().getMapaCalles().keySet());
    ...
    private void guardarEstadoObjetosUsados() {
        ...
        if (num != null && num != "") {
            domicilio.setNumero(num.toString());
        } else {
            domicilio.setNumero(null);
        }
        
        if (piso != null && piso != "") {
            domicilio.setPiso(piso.toString());
        } else {
            domicilio.setPiso(null);
        }
        if (depto != null && depto != "") {
            domicilio.setDepartamento(depto.toString());
        } else {
            domicilio.setDepartamento(null);
        }
        if (barrio != null && barrio != "") {
            domicilio.setBarrio(barrio.toString());
        } else {
            domicilio.setBarrio(null);
        }
        if (manz != null && manz != "") {
            domicilio.setManzana(manz.toString());
        } else {
            domicilio.setManzana(null);
        }
        if (torre != null && torre != "") {
            domicilio.setTorre(torre.toString());
        } else {
            domicilio.setTorre(null);
        }
        if (esca != null && esca != "") {
            domicilio.setEscalera(esca.toString());
        } else {
            domicilio.setEscalera(null);
        }
        if (sec != null && sec != "") {
            domicilio.setSector(sec.toString());
        } else {
            domicilio.setSector(null);
        }
        if (localidad.getIdLocalidad() == -1) {
            localidad = null;
        }
        ...
        if(localidad != null
            && localidad.getIdLocalidad() == localidadDefecto.getIdLocalidad() 
                ...
                if(this.ddCalleFinaliza.getSelected() != null 
...
//        String calleText = null;
//        if (relacionCalle == null && calle != null) calleText = calle.toString();
//        domicilio.setCalle(calleText);
...
//            if (seleccionado instanceof Localidad) {
//                this.cargarValoresPorDefecto();
//            }
        }
...
//        this.ddCalle.setSelected(relacionCalle.getNombre());
//        this.ddCalleComienza.setSelected(relacionCalleComienza.getNombre());
//        System.out.println("LAS OPCIONES ANTES DE SETEAR LA CALLEF SON: " + this.ddCalleFinalizaOptions.getOptions().length);
//        this.ddCalleFinaliza.setSelected(relacionCalleFinaliza.getNombre());
    ...
    private void cargarComboCallesComienzaCuadras(Calle pCalle){
        ...
        int i = 0;
        System.out.println("Todas las cuadras q lindan con la calle: *" +pCalle + "* son: " + locListaCallesComienzan);
    ...
    public void valueChangeEventDdCalleComienza(ValueChangeEvent event){ 
        ...
        if(this.getDdCalleComienza().getSelected().toString().equals("") ){
            this.getDdCalleFinalizaOptions().setOptions(new Option[0]);
        }else{
            Calle locCalleSeleccionadaComienza = this.getCallePorNombre(this.getDdCalleComienza().getSelected().toString());
            Calle locCalleSeleccionada = this.getCallePorNombre(this.getDdCalle().getSelected().toString());
...
//        this.ddCalleFinalizaOptions.setOptions(new Option[0]);
    ...
    private void cargarComboCallesFinalizaCuadras(Calle pCalle, Calle pCalleComienza){
        ...
        this.getDdCalleFinalizaOptions().setOptions(opCuadras);
    ...
    private void cargarValoresPorDefecto() {
        ...
        try {
            ...
            if (localidad != null) {
                localidadDefecto = localidad;
            }
            if (localidadDefecto != null) {
                this.getElementoPila().getObjetos().set(3, localidadDefecto);
            }
        } catch (Exception ex) {
        }
    ...
    private void cambiarSegunLocalidadPorDefecto(boolean mostrar) {
        ...
        if(relacionCalleComienza == null || relacionCalleComienza.getIdAbstractCalle() == -1){
            if(this.getDdCalleComienza().getSelected() != null && !this.getDdCalleComienza().getSelected().toString().equals("")){
            ...
            }else if(relacionCalleComienza.getIdAbstractCalle() != -1){
                System.out.println("ELSE IF 1");
             ...
             System.out.println("Loc calle Comienza=  " + locCalleComienza );
             if(locCalleComienza != null){
                this.cargarComboCallesFinalizaCuadras(locCalle, locCalleComienza);
                this.ddCalleComienza.setSelected(locCalleComienza.toString());
             }
             
        if(relacionCalleFinaliza == null || relacionCalleFinaliza.getIdAbstractCalle() == -1){
            if(this.getDdCalleFinaliza().getSelected() != null && !this.getDdCalleFinaliza().getSelected().toString().equals("")){
        ...
        }else if(relacionCalleFinaliza.getIdAbstractCalle() != -1){
            System.out.println("ELSE IF 1");
        ...
        System.out.println("Loc calle=  " + locCalleFinaliza );
        if(locCalleFinaliza != null){
            this.ddCalleFinaliza.setSelected(locCalleFinaliza.toString());
        }       
