...
public class Expediente extends NodoExpediente implements Serializable {
	...
	public String getStringProcedimiento() {
		return this.nodoProcedimiento != null ? ((Procedimiento) this.nodoProcedimiento)
				.getNombre() : "";
	}
	...
	public List<Documento> listarDocumentacionPresentada() {
		...
		for (NodoExpediente cadaNodoF : listaNodosExpedientes) {
			for (NodoExpediente cadaNodoT : cadaNodoF.getListaNodosExpedientes()) {
				...
				for (Documento cadaDocumento : cadaTramite.getListaDocumentos()) {
	...
	private void cambiarFaseActiva(Fase locNuevaActiva, Fase locActiva, 
			AccionFase pAccion, Usuario pUsuario, String pComentario) {
		...
		if (locNuevaActiva != null) {
			...
			locNuevaActiva
					.setEstado(com.trascender.expedientes.recurso.persistent.Fase.Estado.ABIERTO);
	...
	public List<NodoExpediente> getListaFasesOrdenadasSegunPermisos() {
		...
		for (NodoExpediente cadaNodo : listaNodosExpedientes) {
			...
			if (cadaFase.getFaseAVolver() != null && 
				!cadaFase.equals(this.faseActualSegunPermisos)) {
					continue;
			}
	...
	public boolean isEnPrimerFase(){
		...
		if (this.getListaFasesOrdenada().isEmpty()) return true;
	...
	/**
	 * Setea el atributo faseActualSegunPermisos segun corresponda 
	 * al usuario ver o no la Fase actual del Expediente.
	 * @param pUsuario
	 */
		...
		if (faseActual != null) {
			if (faseActual.getFaseAVolver() != null) {
				...
				if (nodoProcedimiento.getResponsable().esResponsable(pUsuario)
						...
						|| 
						(faseActual.getNodoProcedimiento().getResponsable() != null 
						&& (faseActual.getNodoProcedimiento().getResponsable().esResponsable(pUsuario)
						|| faseActual.getNodoProcedimiento().getResponsable().esSupervisor(pUsuario))
						)
						|| 
						(faseActual.getFaseAVolver().getNodoProcedimiento().getResponsable() != null 
						&& (faseActual.getFaseAVolver().getNodoProcedimiento().getResponsable().esResponsable(pUsuario)
						|| faseActual.getFaseAVolver().getNodoProcedimiento().getResponsable().esSupervisor(pUsuario)))
						) {
	...
	public String toString() {
		String retorno = " Expediente:  " + this.nroRegistro + " : "
				+ ((Procedimiento) this.nodoProcedimiento).getNombre();
		return retorno;
	}
